...
- **Intro overlay scrim lightened (`bg-white/25` → `bg-white/60`)** — `ConfiguratorShell.tsx`'s intro overlay layers a blurred static screenshot (`/intro-bg.jpg`, generated from the default red theme) under a white scrim. That screenshot is the same file for every partner, so its red-tinted blur read as mismatched "random blotching" once seen through a partner's own accent color (e.g. Partner One's blue) rather than a soft, generic frosted-glass hint. Raising the scrim's opacity washes the underlying colors out further, reading as a fainter, more neutral blur regardless of theme — a real fix for the color-mismatch, not just the default view. Applies to all three (default + both partners) since the scrim isn't partner-conditional. If the mismatch is still noticeable after this, the more thorough fix would be moving off one shared static screenshot entirely (e.g. back to a live `backdrop-blur` of the real DOM, which always matches whatever theme is live) — flagged here, not done, since it's a bigger change than what was asked for this pass.
