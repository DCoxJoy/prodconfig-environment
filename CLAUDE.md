...
- **`embed.js` FAB two-line stacking in the "laptop" range (641–1475px)** — a new `@media (min-width: 641px) and (max-width: 1475px)` rule narrows `.agc-fab` to `width: 90px` (auto height, `min-height: 48px`) and sets `.agc-fab-label { white-space: normal; text-align: center }`, wrapping "Bundle Builder" onto two lines ("Bundle"/"Builder") instead of staying a wide single-line pill. Matches the same two-line treatment the site's own header buttons ("Contact Sales" → "Contact"/"Sales", "Shop Now" → "Shop"/"Now") already use at this width — without it, the wider single-line Bundle Builder pill was crowding the main nav (Products/Brands/Verticals/Company/Resources) into wrapping onto a second row at laptop viewport widths. 1400px was tried first (the width where the nav was reported piling up) but the site's own buttons weren't yet stacked there; bumped through 1428px and 1434px to 1475px, confirmed against the live site as the width where "Contact Sales"/"Shop Now" actually wrap. Outside this range: below 641px the existing icon-only mobile rule takes over (label hidden entirely); above 1475px it's the normal single-line pill. Verified with Playwright: 1475px → 90×52 two-line button; 1476px → 140×48 single-line pill.
