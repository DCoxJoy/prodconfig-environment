...
- **`embed.js` FAB two-line stacking in the "laptop" range (641–1400px)** — a new `@media (min-width: 641px) and (max-width: 1400px)` rule narrows `.agc-fab` to `width: 90px` (auto height, `min-height: 48px`) and sets `.agc-fab-label { white-space: normal; text-align: center }`, wrapping "Bundle Builder" onto two lines ("Bundle"/"Builder") instead of staying a wide single-line pill. Matches the same two-line treatment the site's own header buttons ("Contact Sales" → "Contact"/"Sales", "Shop Now" → "Shop"/"Now") already use at this width — without it, the wider single-line Bundle Builder pill was crowding the main nav (Products/Brands/Verticals/Company/Resources) into wrapping onto a second row at 1400px viewport width. 1400px was chosen to match the exact width where the nav was reported piling up; may need adjusting if the site's own breakpoint for its button stacking sits at a slightly different value. Outside this range: below 641px the existing icon-only mobile rule takes over (label hidden entirely); above 1400px it's the normal single-line pill. Verified with Playwright: 1400px → 90×52 two-line button; 1450px → 140×48 single-line pill; 800px → still two-line (90×52); 600px → icon-only (48×48).
