...
- **`embed.js` FAB height matched to header CTAs + red hover-invert** — `.agc-fab` height/radius went from `56px`/`28px` to `48px`/`24px` to match the site's "Contact Sales"/"Shop Now" header buttons (the icon-only collapsed states — `.agc-active` and the mobile-forced circle — were updated to `width: 48px` alongside, so they stay perfect circles at the new size, not ovals). Background went from the blue gradient to `white` with a `1.5px solid #c8291c` border (brand red, same token as `--color-brand`) — an outline-style pill. `color: #c8291c` on `.agc-fab` drives both the label (`.agc-fab-label { color: inherit }`) and the icon (`currentColor` stroke) together; `.agc-fab:hover` fills the background solid red and flips `color` to white, so label and icon invert together on hover. The collapsed circle states (mobile idle, panel-open) read as a white circle with a red outline/icon — verified legible against a white header background. Verified with Playwright against mocked 48px-tall Contact Sales/Shop Now buttons: heights match exactly, idle background is white with red label (`rgb(200,41,28)`), hover background/label invert to red/white.
