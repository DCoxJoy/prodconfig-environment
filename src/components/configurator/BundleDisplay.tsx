  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="text-center sm:text-left space-y-2">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-[#DB0032]/10 text-[#DB0032] border border-[#DB0032]/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {bundle.products.map((product) => (
          <div
            key={product.sku}
            className="group flex flex-col rounded-2xl border border-slate-200 bg-white overflow-hidden hover:border-slate-300 hover:shadow-md transition-all duration-300"
          >
            {/* Visual Image Placeholder */}
            <div className="h-40 bg-slate-50 flex items-center justify-center border-b border-slate-100 relative">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(219,0,50,0.03),rgba(255,255,255,0))]"></div>
              
              {/* Product Shape Sketch (CSS/SVG) */}
              <div className="w-16 h-16 rounded-xl bg-white flex items-center justify-center border border-slate-200 text-slate-400 group-hover:text-[#DB0032] group-hover:border-[#DB0032]/30 transition-all duration-300 shadow-sm">
            ...
            <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
              <div className="space-y-1">
                ...
                <h4 className="font-semibold text-slate-800 group-hover:text-slate-900 line-clamp-1 transition-colors">
                  {product.name}
                </h4>
              </div>
      ...
      <div className="p-6 rounded-2xl border border-[#DB0032]/20 bg-red-50/30 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
          <div className="text-xs font-bold text-[#DB0032] uppercase tracking-wider">
            Total Package Price
          </div>
